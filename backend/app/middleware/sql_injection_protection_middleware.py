...
from app.core.sql_injection_detector import detect_sql_injection_payload
...
SQLI_DUPLICATE_COOLDOWN_SECONDS = 300
...
class SQLInjectionProtectionMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        detected_patterns = detect_sql_injection_payload(request_text)

        if detected_patterns:
            ip_address = self._get_client_ip(scope)

            self._create_sql_injection_incident(
            ...
            response = JSONResponse(
                status_code=403,
                content={
                    "detail": "Запрос заблокирован системой защиты от SQL-инъекций",
    ...
    def _recent_sql_injection_incident_exists(
        self,
        db,
        ip_address: str,
    ) -> bool:
        cooldown_start = datetime.now(timezone.utc) - timedelta(
            seconds=SQLI_DUPLICATE_COOLDOWN_SECONDS
        )

        incident = (
            db.query(SecurityIncident)
            .filter(
                SecurityIncident.incident_type == "SQL Injection Attempt",
    ...
    def _create_sql_injection_incident(
        self,
        ip_address: str,
        method: str,
        path: str,
        ...
        try:
            if self._recent_sql_injection_incident_exists(
                db=db,
                ip_address=ip_address,
            ):
                security_logger.warning(
                    f"SQL INJECTION blocked without duplicate incident | "
                    f"ip={ip_address} | "
                    f"method={method} | "
                    f"path={path} | "
            ...
            security_logger.error(
                f"Failed to create SQL injection incident | "
                f"ip={ip_address} | "
                f"method={method} | "
                f"path={path} | "
                f"error={error}"
